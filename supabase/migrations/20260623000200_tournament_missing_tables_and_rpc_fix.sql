-- ============================================================================
-- Recuperação: tabelas de torneio faltantes + RPCs com auth corrigida
--
-- O VPS tinha tournaments/tournament_participants mas não tournament_matches
-- (migration anterior marcada como [skip] mas tabela não criada).
-- Esta migration é idempotente (IF NOT EXISTS / OR REPLACE).
-- ============================================================================
...
create index if not exists tournament_matches_status
  on tournament_matches(status);
create index if not exists tournament_matches_group
...
-- ── tournament_group_slots (engine v2) ────────────────────────────────────────
...
-- ── tournament_standard_order (seeding espelhado) ────────────────────────────
...
-- ── Helpers internos (clear_forward, clear_group_slots, auto_advance, sync_placement)
-- Recriados aqui para garantir existência no VPS ──────────────────────────────
...
begin
  ...
  if v_match.next_match_id is not null then
    ...
    -- Propaga recursivamente apenas se o vencedor antigo havia avançado
    if exists (
      select 1 from tournament_matches
      where id = v_match.next_match_id
        and winner_participant_id = p_old_winner
    ) then
      update tournament_matches
...
begin
  for v_slot in
    select * from tournament_group_slots
    where tournament_id = p_tournament and group_id = p_group_id
  loop
    update tournament_matches
      set participant_a_id = case when match_slot = 0 then null else participant_a_id end,
          participant_b_id = case when match_slot = 1 then null else participant_b_id end,
          status = 'pending',
...
begin
  ...
  from tournament_matches
  where tournament_id = p_tournament and group_id = p_group_id
    and bracket = 'group' and status = 'finished';

  if v_total = 0 or v_done < v_total then return; end if;

  for v_slot in
    select gs.* from tournament_group_slots gs
    where gs.tournament_id = p_tournament and gs.group_id = p_group_id
    order by gs.rank
  loop
    ...
    join (
      select winner_participant_id,
             row_number() over (order by
               sum(case when winner_participant_id = tp2.id then 3 else 0 end) desc,
               sum(case when participant_a_id = tp2.id then score_a else score_b end
                   filter (where status = 'finished')) -
               sum(case when participant_a_id = tp2.id then score_b else score_a end
                   filter (where status = 'finished')) desc
             ) as pos
      ...
      group by tp2.id
    ) standings on standings.winner_participant_id = tp.id
    where tp.tournament_id = p_tournament and tp.group_id = p_group_id
      and standings.pos = v_slot.rank + 1
    limit 1;
...
begin
  ...
  into v_semi_a, v_semi_b, v_loser_a
  ...
  where m.tournament_id = p_tournament
    ...
    and m.next_match_slot = 0
  limit 1;

  select case when m.winner_participant_id = m.participant_a_id then m.participant_b_id else m.participant_a_id end
  into v_loser_b
  from tournament_matches m
  where m.tournament_id = p_tournament
    and m.bracket = 'winners'
    and m.next_match_id = (select id from tournament_matches where tournament_id = p_tournament and bracket = 'winners' and round = 1 limit 1)
    and m.next_match_slot = 1
  limit 1;

  update tournament_matches
    set participant_a_id = v_loser_a,
        participant_b_id = v_loser_b,
        status = case
          when v_loser_a is not null and v_loser_b is not null then 'scheduled'::match_status
...
-- ── generate_bracket (service_role-aware) ─────────────────────────────────────
...
begin
  ...
  if v_t.format = 'groups_knockout' then
    ...
    from tournament_participants tp
    where tp.tournament_id = p_tournament and tp.group_id is not null and tp.signup_status = 'confirmed';
    ...
    from (select count(*)::int as cnt from tournament_participants
          where tournament_id = p_tournament and signup_status = 'confirmed' and group_id = any(v_group_ids)
          group by group_id) t;
    ...
    if v_n <> v_total_class then
      raise exception 'Configuração inválida: % classificados não é potência de 2', v_total_class;
    end if;
    v_slot := 0;
    ...
    for r in 1..v_km_rounds loop
      for i in 0..( (2^(v_km_rounds - r + 1) / 2)::int - 1 ) loop
        if r > 1 then v_next_id := v_km_ids[(2^(r-2))::int + i/2]; v_next_slot := i % 2;
        else v_next_id := null; v_next_slot := null; end if;
    ...
    for r in 0..(v_spots - 1) loop
      if r % 2 = 0 then v_tier_groups := v_group_ids;
        ...
        v_rev_groups := '{}';
        for i in reverse array_length(v_group_ids,1)..1 loop v_rev_groups := v_rev_groups || v_group_ids[i]; end loop;
        v_tier_groups := v_rev_groups;
      end if;
      if r = 0 then
        for i in 1..array_length(v_tier_groups,1) loop
          v_seed_groups := v_seed_groups || v_tier_groups[i]; v_seed_ranks := v_seed_ranks || 0;
        end loop;
      else
    ...
    for i in 0..(v_total_class/2 - 1) loop
      declare
        ...
        v_grp_a text := v_seed_groups[i*2+1]; v_rnk_a int := v_seed_ranks[i*2+1];
        v_grp_b text := v_seed_groups[i*2+2]; v_rnk_b int := v_seed_ranks[i*2+2];
      begin
        ...
        on conflict (tournament_id,group_id,rank) do update set match_id=excluded.match_id, match_slot=excluded.match_slot;
        insert into tournament_group_slots (tournament_id,group_id,rank,match_id,match_slot)
  ...
  if v_t.format = 'king_of_table' then
    ...
    for i in 1..(v_n_actual-1) loop
      ...
      values (v_km_ids[i], p_tournament, i, 'winners', 0,
              case when i=1 then v_parts[1] else null end, v_parts[i+1],
              case when i < v_n_actual-1 then v_km_ids[i+1] else null end, 0,
  ...
  v_total  := v_n - 1;
  v_ids := '{}';
  for i in 1..v_total loop v_ids := v_ids || gen_random_uuid(); end loop;
  v_order := tournament_standard_order(v_n);
  for r in 1..v_rounds loop
    for i in 0..((2^(r-1))::int - 1) loop
      if r > 1 then v_next_id := v_ids[(2^(r-2))::int + i/2]; v_next_slot := i % 2;
      else v_next_id := null; v_next_slot := null; end if;
      if r = v_rounds then
      ...
      if v_is_bye and v_winner is not null and v_next_id is not null then
        update tournament_matches
          ...
          where id = v_next_id;
        update tournament_matches set status = 'scheduled'
          where id = v_next_id and participant_a_id is not null and participant_b_id is not null and status = 'pending';
      end if;
    end loop;
  end loop;
  if v_t.third_place_match and v_n_actual >= 4 then
    insert into tournament_matches (tournament_id, round, bracket, slot, status)
    values (p_tournament, 1, 'placement', 1, 'pending');
  end if;
  update tournaments set status = 'active' where id = p_tournament;
  return query select * from tournament_matches where tournament_id = p_tournament order by bracket desc, round desc, slot;
end;
...
-- ── report_match_result (service_role-aware) ──────────────────────────────────
...
begin
  ...
  update tournament_matches
    set score_a=p_a, score_b=p_b, sets=p_sets, winner_participant_id=v_new_winner, status='finished', finished_at=now()
    where id=p_match returning * into v_match;
  if v_match.next_match_id is not null then
    update tournament_matches
      ...
      where id=v_match.next_match_id returning * into v_next;
...
-- ── revert_match_result (service_role-aware) ─────────────────────────────────
...
begin
  ...
  update tournament_matches
    set score_a=null, score_b=null, sets=null, winner_participant_id=null, finished_at=null,
...
begin
  ...
  return report_match_result(p_match,
...
-- ── close_group_stage (service_role-aware) ────────────────────────────────────
...
begin
  if not is_admin_caller() then raise exception 'Acesso negado'; end if;
  for v_group_id in
    select distinct group_id from tournament_group_slots
    where tournament_id=p_tournament order by group_id
  loop
...
-- ── tournament_standings view (engine v2) ─────────────────────────────────────
