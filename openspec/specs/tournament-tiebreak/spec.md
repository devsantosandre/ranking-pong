...
## Purpose
Classificação de fase de grupos com desempate oficial ITTF/CBTM: pontuação e ordenação primária por pontos de vitória, captura de placar set-a-set nos jogos de grupo e desempate progressivo entre empatados (razão de sets → razão de pontos de game), calculado de forma única (mesma ordenação na exibição e na definição dos classificados que avançam ao mata-mata).
